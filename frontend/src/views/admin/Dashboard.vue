...
<template>
  ...
  >
    <section class="space-y-6">
      ...
      <div
        v-if="isEmpty"
        class="rounded-[28px] border border-dashed border-[#B9D7DF] bg-[#F7FCFD] p-8 text-center"
      >
      ...
      <div class="grid gap-6 2xl:grid-cols-[minmax(0,1.8fr)_360px]">
        <div class="space-y-6">
          <div class="grid gap-6 xl:grid-cols-[minmax(0,1.35fr)_minmax(300px,1fr)]">
            <div class="space-y-4">
              <TrendChart
                :labels="patientVisitTrend.labels"
                :values="patientVisitTrend.values"
                :loading="loading"
              />
              <div
                v-if="!loading && hasData && isTrendEmpty"
          ...
          <AppointmentTable
            v-if="loading || hasRecentAppointments"
